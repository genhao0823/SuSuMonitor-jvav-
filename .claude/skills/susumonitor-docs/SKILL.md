...
description: SuSuMonitor 文档与 Java 后端规范。当在 SuSuMonitor 项目目录下编写开发计划、开发日志、OpenAPI、协议文档或 Java/Spring Boot 后端代码时自动加载。
when_to_use: 编写开发计划 / 开发日志 / OpenAPI / 协议文档 / Java 后端代码 时
user-invocable: true
---
...
- 文件命名格式: `openapi-{模块名}.json`（如 `openapi-alert.json`）
- 每次新增或修改 API 接口时，同步更新对应的 OpenAPI 文档
- 模板见 `references/openapi-template.json`
- 前端 `npm run openapi:check` 强制校验契约与 Java Controller 同步
...
- Java 代码中的每一个注解前必须添加注释，说明该注解的作用和用法
- 注解注释必须说明该注解如何影响当前类、方法、字段或参数，以及必要的生效条件
- 注释不能只写"参数校验"或"接口映射"等泛化描述，必须说明具体行为
- 多个连续注解应在每个注解前分别添加对应注释，不得使用一条注释笼统覆盖多个注解

### 代码说明规范

- 每一个类、接口和方法都必须添加注释，说明其职责、作用和使用场景
- 重要代码段必须添加注释，说明其业务目的、关键处理逻辑和必要的前置条件
- 注释应解释"为什么这样实现"和"代码产生什么影响"，不得只重复代码字面含义
- 注释必须与代码保持同步；修改代码行为时必须同步更新相关注释

示例：

```java
// 将类注册为 REST Controller，并将返回值序列化为 HTTP 响应体。
@RestController
// 为当前 Controller 下的接口统一增加 /api 前缀。
@RequestMapping("/api")
public class SystemController {

    // 校验字符串不能为空、不能是空字符串或只包含空白字符。
    @NotBlank
...
- 所有 REST API 使用统一响应结构（`ApiResponse<T>`）
...
- 服务器删除采用软删除，不物理删除记录
- Token 有效期 72 小时（与 `application.yml` 对齐）
