---
name: susumonitor-mysql
description: SuSuMonitor MySQL 规范。当在 SuSuMonitor 项目 server-java-SuMon/ 目录下进行数据库设计或编写 SQL 时自动加载。触发场景：数据库表设计、编写 SQL 语句、索引设计、Flyway 迁移脚本。
when_to_use: 设计数据库表 / 编写 SQL / 设计索引 / 编写 Flyway 迁移 时
user-invocable: true
---

# SuSuMonitor MySQL 规范
...
- 敏感字段（密码、Token、SSH 私钥）只能查密文 / 哈希，不能明文落库
...
- 破坏性操作（DROP/TRUNCATE/批量 DELETE）执行前必须：
  1. 备份并验证备份可读
  2. 展示影响范围并获得最终确认
  3. 操作后验证数据库结构与数据状态
...
- 凭据（密码 / 私钥 / 私钥口令）只存 AES-256-GCM 密文，对称密钥从环境变量读取
