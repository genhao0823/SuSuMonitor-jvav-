---
name: susumonitor-git
description: SuSuMonitor Git 提交规范。当在 SuSuMonitor 项目目录下进行 Git 操作时自动加载。触发场景：提交代码、创建分支、编写 commit message、合并代码。
when_to_use: 提交代码 / 创建分支 / 编写 commit message / 合并代码 时
user-invocable: true
---

# SuSuMonitor Git 提交规范
...
| agent | Agent 采集程序（Go） |
| web | Web 前端（Vue 3） |
| android | Android 客户端（Kotlin，启用时） |
| skills | 项目级 skill 文件 |
| docs | 文档 |
| deploy | 部署相关 |
...
## 4. 前置检查（commit / push 前必看）

- `git status` 确认 working tree 干净或只含本模块预期改动
- `git diff` 确认无意外修改
- 目标分支正确（不要误推到 main / 别人分支）
- remote 配置正确
- 未跟踪文件已被有意处理（不是构建产物或敏感文件）
- 没有 `.env` / 密码 / Token / 证书 / 私钥 / 数据库备份
...
- pre-commit 钩子（如有）失败时必须调查根因，不得使用 `--no-verify` 绕过
- prefer 提交而不是 amend（保留清晰的 commit 历史）
